...
{
     do {
	  success = MasterReceive( 0xb0, &statusCode, 1 );
...
void read_and_send(uint8_t whichversion)
{
  ...
  cycle_reset();
  get_slave_status();                  
  success = MasterTransmit( 0xb0, runApp, 2 );
  read_from_slave();
  sendchar( statusCode );		  
} 
...
__C_task void main(void)
{
	...
	{      
		...
		sendchar('Y'); 
	      ...
	      {   
                  addr =(recchar()<<8) | recchar(); 
		  addr = addr<<1;
                  if(addr > 0X1800) over_size_flag =1;
		 ...
		 sendchar('\r'); 
		  ...
		  val = recchar(); 
		  sendchar( BlockLoad(temp_int,val) ); 
                   if(reps == 0)
                      First_Time();
                  success = MasterTransmit( 0xb0, pageBuffer, 64+3 );
                  if (success) 
                  {
                              get_slave_status();
                  }           
